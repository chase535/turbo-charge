...
int main()
{
    ...
    while(1)
    {
        ...
        snprintf(current_max_char, 20, "%u", options_value[4]);
        snprintf(highest_temp_current_char, 20, "%u", options_value[9]);
        ...
        if(!battery_status)
        {
            ...
            if(step_charge == 1)
            {
                if(options_value[1] == 1) (atoi(power) < (int)options_value[5])?step_charge_ctl("1"):step_charge_ctl("0");
                else step_charge_ctl("1");
            }
            else if(step_charge == 2)
                (options_value[1] == 1)?step_charge_ctl("0"):step_charge_ctl("1");
            sleep(options_value[0]);
            continue;
        }
        ...
        if(strcmp(charge, "Discharging"))
        {
            ...
            if(options_value[2] == 1 && temp_sensor_num != 100 && current_change)
            {
                check_read_file(temp_sensor);
                ...
                if(temp_int >= ((int)options_value[8])*1000)
                {
                    snprintf(chartmp, PRINTF_WITH_TIME_MAX_SIZE, "手机温度大于等于降低充电电流的温度阈值，限制充电电流为%dμA", options_value[9]);
                    printf_with_time(chartmp);
                    while(1)
                    {
                        ...
                        if(tmp[3])
                        {
                            if(temp_int < ((int)options_value[8])*1000)
                            {
                                snprintf(chartmp, PRINTF_WITH_TIME_MAX_SIZE, "新的降低充电电流的温度阈值高于旧的温度阈值，且手机温度小于新的温度阈值，恢复充电电流为%dμA", options_value[4]);
                                printf_with_time(chartmp);
                                break;
                            }
                            else
                            {
                                snprintf(chartmp, PRINTF_WITH_TIME_MAX_SIZE, "新的降低充电电流的温度阈值高于旧的温度阈值，但手机温度大于等于新的温度阈值，限制充电电流为%dμA", options_value[9]);
                                printf_with_time(chartmp);
                            }
                            tmp[3]=0;
                        }
                        ...
                        if(!strcmp(charge, "Discharging"))
                        {
                            snprintf(chartmp, PRINTF_WITH_TIME_MAX_SIZE, "充电器断开连接，恢复充电电流为%dμA", options_value[4]);
                            printf_with_time(chartmp);
                            tmp[0]=1;
                            tmp[1]=0;
                            break;
                        }
                        if(temp_int <= ((int)options_value[10])*1000)
                        {
                            snprintf(chartmp, PRINTF_WITH_TIME_MAX_SIZE, "手机温度小于等于恢复快充的温度阈值，恢复充电电流为%dμA", options_value[4]);
                            printf_with_time(chartmp);
                            break;
                        }
                        if(!options_value[2])
                        {
                            snprintf(chartmp, PRINTF_WITH_TIME_MAX_SIZE, "温控关闭，恢复充电电流为%dμA", options_value[4]);
                            printf_with_time(chartmp);
                            break;
                        }
                        if(step_charge == 1)
